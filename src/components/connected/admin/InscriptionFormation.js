import React from 'react';

function InscriptionFormation() {

  return (
    <div className="formation">
      <div className="rf-container-fluid">
        <h4>Présentation de l&rsquo;offre de formation des Conseillers numériques France services</h4>
        <div className="rf-grid-row">
          <a href="https://cdn.conseiller-numerique.gouv.fr/Presentation_offre_de_formation.pdf"
            className="rf-link" target="blank" rel="noreferrer" title="Offre de formation">
              Télécharger la présentation de l&rsquo;offre de formation (format PDF)
          </a>
        </div>
      </div>
    </div>
  );
}

export default InscriptionFormation;
