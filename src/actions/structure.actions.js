...
function getAll({ departement = null, region = null, page = 0, filter, sortData = 'createdAt', sortOrder = 1 }) {
  return dispatch => {
    dispatch(request());

    structureService.getAll(departement, region, page, filter, sortData, sortOrder)
    .then(
